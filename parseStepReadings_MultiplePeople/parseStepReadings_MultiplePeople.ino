...
int on_step1 = 1, on_step2 = 1, on_step3 = 1;
...
void check_step2_retrospect(){
  ...
  if(on_step2 >= 1 && weight_sum > (VALUE_PER_PERSON * on_step2 * 2)){
    count++;
...
//      print_array(2);
  ...
  if(on_step2 > 1 && (weight_sum < (VALUE_PER_PERSON * on_step2))){
    count++;
    if(count > COUNT_THRESHOLD){
      on_step2 -= 1;
      count = 0;
...
void check_step(float step_number){
  ...
  if(step_number == 1){
    ...
    if(on_step1 >= 1 && weight > (VALUE_PER_PERSON * on_step1)){  
      count ++;
      if(count > COUNT_THRESHOLD){
        on_step1 += 1;
        count = 0;
        str = "1 n " + String(micros()) + " " + String(going_in) + " " + String(weight);
        Serial.println(str);
//        print_array(step_number);
      }
    }
    if(on_step1 > 1 && weight < (VALUE_PER_PERSON * on_step1 * 0.5)){ 
      count ++;
      if(count > COUNT_THRESHOLD){      
        on_step1 -= 1;
        count = 0;
        str = "1 f " + String(micros()) + " " + String(going_in) + " " + String(weight);
        Serial.println(str);
//        print_array(step_number);
      }      
    }
  ...
  if(step_number == 3){
    ...
    read_into_array(step_number, weight);
    if(on_step3 >= 1 && weight > (VALUE_PER_PERSON * on_step3)){  
      count ++;
      if(count > COUNT_THRESHOLD){
        on_step3 += 1;
        count = 0;
        str = "3 n " + String(micros()) + " " + String(going_in) + " " + String(weight);
        Serial.println(str);
//        print_array(step_number);
      }
    }
    if(on_step3 > 1 && weight < (VALUE_PER_PERSON * on_step3 * 0.5)){ 
      count ++;
      if(count > COUNT_THRESHOLD){      
        on_step3 -= 1;
        count = 0;
        str = "3 f " + String(micros()) + " " + String(going_in) + " " + String(weight);
        Serial.println(str);
//        print_array(step_number);
      }      
    }
